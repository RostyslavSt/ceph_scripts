...
env.roledefs['gitbuilder_auto'] = [
    'ubuntu@gitbuilder-ceph-deb-trusty-cmake.mirantis.com'
                ...
                with cd('/srv'):
                    ...
                    with cd('gnupg'):
                        if not exists('pubring.gpg'):
                            ...
                            put('pubring.gpg' '/srv/gnupg/pubring.gpg')
                            put('secring.gpg' '/srv/gnupg/secring.gpg')
                            put('trustdb.gpg' '/srv/gnupg/trustdb.gpg')
#                            put('trustdb.gpg')
                            sudo("cp /home/ubuntu/*.gpg ./")
    ...
    with cd('/srv'):
        ...
        with cd('/srv/gnupg'):
            if not exists('pubring.gpg', use_sudo=True):
                # put doesn't honor cd() for some reason
                put('trustdb.gpg' '/srv/gnupg/trustdb.gpg', use_sudo=True)
                put('pubring.gpg' '/srv/gnupg/pubring.gpg', use_sudo=True)
                put('secring.gpg' '/srv/gnupg/secring.gpg', use_sudo=True)
...
def _gitbuilder_ceph(flavor):
    _gitbuilder(
        flavor=flavor,
        git_repo='https://github.com/Mirantis/ceph.git',
        extra_remotes=dict(
...
def gitbuilder_auto():
    _deb_builder('https://github.com/Mirantis/ceph.git', 'auto',
                 extra_remotes=dict(
                     ci='https://github.com/ceph/ceph-ci.git'
                 ))
    sudo('start autobuild-ceph || /etc/init.d/autobuild-ceph start ; systemctl enable autobuild-ceph || true ; systemctl start autobuild-ceph || true')
...
def gitbuilder_ceph_rpm():
    _gitbuilder_ceph_rpm('https://github.com/Mirantis/ceph.git', 'auto',
                         extra_remotes=dict(
                             ci='https://github.com/ceph/ceph-ci.git'
                         ))
